...
<head>
    ...
    <title>Lupa Kata Sandi - {{ \App\Models\Setting::where('key', 'store_name')->value('value') ?? \App\Models\Setting::where('key', 'web_name')->value('value') ?? 'Rayzell Store' }}</title>
    ...
    <div class="w-full max-w-md space-y-6">
        ...
        <div class="w-full max-w-md space-y-8 bg-white dark:bg-slate-800 rounded-2xl p-8 border border-gray-200 dark:border-slate-700 shadow-2xl relative overflow-hidden transition-colors duration-300">
            ...
            <form method="POST" action="/forgot-password" class="space-y-4 relative z-10">
                ...
                @if (session('success'))
                    <div class="bg-emerald-500/10 border border-emerald-500/20 text-emerald-600 dark:text-emerald-400 p-3 rounded-xl text-xs font-semibold">
                        {{ session('success') }}
                    </div>
                @endif
